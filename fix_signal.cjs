...
const updated = content.replace(/\/\/ Prevent duplicates: Only 1 signal per day untuk zone harga yang sama[\s\S]*?if \(duplicateTodaySignals\.length >= 1\)/, `// Prevent duplicates: Only 1 signal per day untuk zone harga yang sama (supaya signal fresh)
