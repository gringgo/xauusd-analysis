...
*/
  ...
  async function sendWhatsAppAlert(signal: any) {
    ...
    const entryNum = Number(signal.entryPrice) || Number(signal.entryRange) || 0;
    const isBuy = signal.direction === 'BUY';

    const slVal = isBuy ? (entryNum - 5.0).toFixed(2) : (entryNum + 5.0).toFixed(2);
    const tp1Val = isBuy ? (entryNum + 4.0).toFixed(2) : (entryNum - 4.0).toFixed(2);
    const tp2Val = isBuy ? (entryNum + 5.0).toFixed(2) : (entryNum - 5.0).toFixed(2);
    const tp3Val = isBuy ? (entryNum + 6.0).toFixed(2) : (entryNum - 6.0).toFixed(2);
    const tp4Val = isBuy ? (entryNum + 7.0).toFixed(2) : (entryNum - 7.0).toFixed(2);
    const tp5Val = isBuy ? (entryNum + 8.0).toFixed(2) : (entryNum - 8.0).toFixed(2);
    const tp6Val = isBuy ? (entryNum + 9.0).toFixed(2) : (entryNum - 9.0).toFixed(2);
    const tp7Val = isBuy ? (entryNum + 10.0).toFixed(2) : (entryNum - 10.0).toFixed(2);

    const actualSl = signal.sl ? Number(signal.sl).toFixed(2) : slVal;
