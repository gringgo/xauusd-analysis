...
async function backgroundWeeklySync() {
  ...
  async function sendWhatsAppAlert(signal: any) {
    ...
    let message = `*XAUUSD SIGNAL* 🚨\n\n`;
    message += `*Jenis Signal:* ${signal.type} (${signal.timeframe})\n`;
    message += `*Trigger:* ${emoji} ${signal.direction}\n`;
  ...
  async function sendTelegramSignalAlert(signal: any) {
    ...
    const emoji = signal.direction === 'BUY' ? '🟢' : '🔴';
    let message = `🚨 <b>XAUUSD SIGNAL</b> 🚨\n\n`;
    message += `🔹 <b>Jenis Signal:</b> ${signal.type} (${signal.timeframe})\n`;
    message += `🔹 <b>Trigger:</b> ${emoji} ${signal.direction}\n`;
    message += `🔹 <b>Entry Zone:</b> ${signal.entryRange}\n`;
    message += `🔹 <b>TP:</b> ${signal.tp}\n`;
  ...
  app.post("/api/signals", async (req, res) => {
    try {
      ...
      // We will set winRate since it is not saved to DB but useful for alerts
      const signalWithWinRate = { ...inserted[0], winRate: newSignal.winRate };
